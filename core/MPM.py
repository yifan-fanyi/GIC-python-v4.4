...
myLog('<Framework Version> core.MPM -> %s'%__version__)
print('<Framework Version> core.MPM -> %s'%__version__)
# 2021.03.24
# @yifan
...
import numpy as np
import copy
from core.util.myIO import bits2int, int2bits
from core.util.mylearner import myLearner
import collections
...
class MPM():
    def __init__(self, n_codeword, MPM_init_list=[0,1,2]):
...
class ML_MPM(MPM):
    def __init__(self, learner, n_codeword):
        super().__init__(n_codeword)
    ...
    def fit(self, idx):
        prev, cur = [], []
        idx = idx.astype('int16')
        counter = collections.Counter(copy.deepcopy(idx).reshape(-1).tolist())
        d = counter.most_common(3)
        self.MPM_init_list = [d[0][0], d[1][0], d[2][0]]
        print('MPM_init_list', self.MPM_init_list)
    ...
    def encode(self, idx):
        ...
        for k in range(idx.shape[0]):
            for i in range(idx.shape[1]):
                for j in range(idx.shape[2]):
                    prev_idx = [self.get_prev(idx[k], i-1, j-1),
                                self.get_prev(idx[k], i-1, j),
                                self.get_prev(idx[k], i, j-1),
                                self.get_prev(idx[k], i-1, j+1)]
                    x.append(prev_idx)
                    y.append(idx[k,i,j,0])
    ...
    def decode(self, stream, K, H, W, raw_idx=None):
        self.stream = stream
        idx = np.zeros((K, H, W, 1))
        self.ct = 0
        prev_idx = []
        for k in range(idx.shape[0]):
            for i in range(idx.shape[0]):
                for j in range(idx.shape[1]):
                    prev_idx = [self.get_prev(idx[k], i-1, j-1),
                                ...
                                self.get_prev(idx[k], i, j-1),
                                self.get_prev(idx[k], i-1, j+1)]
                    idx[k,i,j,0] = self.decode_one_idx(prev_idx)
                    if raw_idx is not None:
                        assert(idx[k,i,j,0] == raw_idx[k,i,j,0]), 'Decoding Error!'
        return idx.astype('int16')

class ML_MPM3(ML_MPM):
    def __init__(self, learner, n_codeword):
        super().__init__(learner, n_codeword)
        self.length = (int)(np.log2(n_codeword-4))+1
    ...
    def encode(self, idx):
        ...
        self.MPM_list = idx[:, -3:]
        print(x.shape, y.shape,px.shape)
        print('test score',self.learner.score(x, y))
        print('test score top3',self.learner.topNscore(x, y, 3))
        for i in range(len(self.MPM_list)):
