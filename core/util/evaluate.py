...
from core.util.bd_rate import BD_PSNR, BD_RATE
from core.util.ssim import structural_similarity, MultiScaleSSIM
...
def PSNR(ref, X, max_val=255):
    if len(X.shape) == 4:
        ...
        return 20*np.log10(max_val/np.sqrt(mean_squared_error(ref, X)))
